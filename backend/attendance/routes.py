...
_session_create_rate_limiter = RateLimiter(window_seconds=3600, max_attempts=30)
_public_status_rate_limiter = RateLimiter(window_seconds=60, max_attempts=60)
_sign_ip_rate_limiter = RateLimiter(window_seconds=300, max_attempts=20)
_sign_session_rate_limiter = RateLimiter(window_seconds=3600, max_attempts=1000)
...
def _get_owned_session(db_session: Session, token: str, current_user: UserModel) -> RecordingSession:
    session = db_session.query(RecordingSession).filter(RecordingSession.token == token).first()
    if session is None or session.organizer_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Session introuvable.")
    return session


def _count_confirmed(db_session: Session, session_id: int) -> int:
    return db_session.query(Signature).filter(Signature.session_id == session_id).count()
...
def get_session_status(
    ...
    session = _get_owned_session(db_session, session_token, current_user)
    confirmed = _count_confirmed(db_session, session.id)
...
def get_public_session(session_token: str, request: Request, db_session: Session = Depends(get_db)):
...
def sign_attendance(
    session_token: str, body: SignRequest, request: Request, db_session: Session = Depends(get_db)
):
